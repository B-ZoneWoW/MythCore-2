...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (GetSecurity() <= SEC_VIP && class_ == CLASS_DEATH_KNIGHT && req_level_for_heroic > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
    ...
    */
    switch (createInfo->Stage)
    {
        ...
        case 3:
        {
            ...
            if (GetSecurity() <= SEC_VIP && createInfo->Class == CLASS_DEATH_KNIGHT && !hasHeroicReqLevel)
            {
                WorldPacket data(SMSG_CHAR_CREATE, 1);
                data << uint8(CHAR_CREATE_LEVEL_REQUIREMENT);
                SendPacket(&data);
