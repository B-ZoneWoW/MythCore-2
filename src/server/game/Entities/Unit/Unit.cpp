...
bool Unit::isSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    if (roll_chance_f(crit_chance) && !pVictim->HasAura(53480))
        return true;
    return false;
}
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (victim->GetVehicle())
        victim->ExitVehicle();
    if (Vehicle* vehKit = victim->GetVehicleKit())
        vehKit->RemoveAllPassengers();
}
...
uint32 Unit::GetRemainingPeriodicAmount(uint64 caster, uint32 spellId, AuraType auraType, uint8 effectIndex) const
{
    ...
    for (AuraEffectList::const_iterator i = periodicAuras.begin(); i != periodicAuras.end(); ++i)
    {
        ...
        if ((*i)->GetTotalTicks() == 0)
            continue;
        
        amount += uint32(((*i)->GetAmount() * std::max<int32>(int32((*i)->GetTotalTicks() - (*i)->GetTickNumber()), 0)) / (*i)->GetTotalTicks());
        //amount += uint32(((*i)->GetAmount() * std::max<int32>(int32((*i)->GetTotalTicks() - int32((*i)->GetTickNumber())), 0)) / (*i)->GetTotalTicks());
