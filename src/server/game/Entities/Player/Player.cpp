        ...
        else if (p->area_level > 0)
        {
            ...
            else
            {
                ...
                if (diff < -5)
                {
                    ...
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(getLevel()+5)*sWorld->getRate(RATE_XP_PREMIUM_EXPLORE));
                    } 
                    else 
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(getLevel()+5)*sWorld->getRate(RATE_XP_EXPLORE));
                ...
                else if (diff > 5)
                {
                    ...
                    if(GetSession()->IsPremium())
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(p->area_level)*exploration_percent/100*sWorld->getRate(RATE_XP_PREMIUM_EXPLORE));
                    } 
                    else 
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(p->area_level)*exploration_percent/100*sWorld->getRate(RATE_XP_EXPLORE));
                ...
                else
                {
                    ...
                    if(GetSession()->IsPremium())
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(p->area_level)*sWorld->getRate(RATE_XP_PREMIUM_EXPLORE));
                    } 
                    else 
                    {
                        XP = uint32(sObjectMgr->GetBaseXP(p->area_level)*sWorld->getRate(RATE_XP_EXPLORE));
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (GetSession()->IsPremium())
    {
        honor_f *= sWorld->getRate(RATE_PREMIUM_HONOR);
    }
    else
    {
        honor_f *= sWorld->getRate(RATE_HONOR);
...
void Player::UpdateZone(uint32 newZone, uint32 newArea)
{
    ...
    // move down for detecting sanctuary area
    // UpdateArea(newArea);
...
void Player::SendLoot(uint64 guid, LootType loot_type)
{
    ...
    else if (IS_CORPSE_GUID(guid))                          // remove insignia
    {
        ...
        if (!bones->lootForBody)
        {
            ...
            if (GetSession()->IsPremium())
            {
                bones->loot.gold = (uint32)( urand(50, 150) * 0.016f * pow( ((float)pLevel)/5.76f, 2.5f) * sWorld->getRate(RATE_PREMIUM_DROP_MONEY) );
            }
            else
            {
                bones->loot.gold = (uint32)( urand(50, 150) * 0.016f * pow( ((float)pLevel)/5.76f, 2.5f) * sWorld->getRate(RATE_DROP_MONEY) );
    ...
    else
    {
        ...
        if (loot_type == LOOT_PICKPOCKETING)
        {
            if (!creature->lootForPickPocketed)
            {
                ...
                if (GetSession()->IsPremium())
                {
                    loot->gold = uint32(10 * (a + b) * sWorld->getRate(RATE_PREMIUM_DROP_MONEY));
                }
                else
                {
                    loot->gold = uint32(10 * (a + b) * sWorld->getRate(RATE_DROP_MONEY));
