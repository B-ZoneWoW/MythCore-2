    ...
    public:
        ...
        struct boss_valithriaAI : public BossAI
        {
            ...
            void DoAction(int32 const action)
            {
                ...
                {
                    Reset();
                    if (Creature* combatTrigger = me->GetCreature(*me, instance->GetData64(DATA_VALITHRIA_TRIGGER)))
            ...
            void CheckHealth()
            {
                ...
                switch (instance->GetBossState(DATA_VALITHRIA_DREAMWALKER))
                {
                    case DONE:
                        return;
                    case NOT_STARTED:
                ...
                if (!bEnd && HealthAbovePct(99))
                {
                    ...
                    me->SummonGameObject(RAID_MODE(GO_DREAMWALKER_CACHE_10_N, GO_DREAMWALKER_CACHE_25_N, GO_DREAMWALKER_CACHE_10_H, GO_DREAMWALKER_CACHE_25_H), 4222.89f, 2484.01123f, 364.871033f, 6.272967f, 0, 0, 1, 1, 604800);
                }
            }
            ...
            void DoAction(int32 const action)
            {
                ...
                if (action == EVENT_SUMMON_DREAM_CLOUD && !(IsHeroic() && bAlreadySummonedClouds))
                {
                    ...
                    for(uint8 i = 0; i <= numberOfClouds; ++i) //need correct count
                    {
                        ...
                        angle = static_cast<float>(2 * M_PI) * i / numberOfClouds + (float)rand_norm() * 0.07f;
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            me->SetVisible(false);
                            //In normal mode, the clouds have a very fast respawn so you wont run the risk of running out of them.
                            if (!IsHeroic())
...
class npc_icc_combat_stalker : public CreatureScript
{
    public:
        ...
        struct npc_icc_combat_stalkerAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                if (!pInstance || pInstance->GetBossState(DATA_VALITHRIA_DREAMWALKER) != IN_PROGRESS)
                    return;
                events.Update(diff);
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_ZOMBIE_AND_ARCHMAGE:
                        {
                            ...
                            uint32 delay = RAID_MODE<uint32>(3000, 15000, 3000, 15000);
                        ...
                        case EVENT_SUMMON_SUPPRESSOR:
                        {
                            ...
                            if (bSummonSuppressors)
                            {
                                ...
                                m_uiSummonSuppressorTimer -= 1700;
                                if (m_uiSummonSuppressorTimer < 17000)
                                    m_uiSummonSuppressorTimer = 30000;
                            }
                        ...
                        case EVENT_SUMMON_BLAZING_SKELETON:
                        {
                            ...
                            if (bSummonSkeletons)
                            {
                                ...
                                m_uiSummonSkeletonTimer -= 2500;
                                if (m_uiSummonSkeletonTimer < 17000)
                                    m_uiSummonSkeletonTimer = 30000;
                            }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            if (Creature *valithria = ObjectAccessor::GetCreature(*me, me->GetInstanceScript()->GetData64(DATA_VALITHRIA_DREAMWALKER)))
                            {
