    ...
    public:
        ...
        struct boss_rotfaceAI : public BossAI
        {
            ...
            void Reset()
            {
                _Reset();
                events1.Reset();
                events1.ScheduleEvent(EVENT_SLIME_SPRAY, 20000);
                events1.ScheduleEvent(EVENT_HASTEN_INFECTIONS, 90000);
                events1.ScheduleEvent(EVENT_MUTATED_INFECTION, 14000);
                infectionStage = 0;
                infectionCooldown = 14000;
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events1.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1, 0.0f, true, -MUTATED_INFECTION, SELECT_ARC_FRONT, static_cast<float>(M_PI/2));
                            if (!target)
                                Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true, -MUTATED_INFECTION, SELECT_ARC_FRONT, static_cast<float>(M_PI/2));
                            if (target)
                                me->CastCustomSpell(SPELL_MUTATED_INFECTION, SPELLVALUE_MAX_TARGETS, 1, target, false);
                            if(target)
                                events1.ScheduleEvent(EVENT_MUTATED_INFECTION, infectionCooldown);
                            else
                                events1.ScheduleEvent(EVENT_MUTATED_INFECTION, 3000);
                            break;
                        }
                        default:
                            break;
...
class npc_little_ooze : public CreatureScript
{
    public:
        ...
        struct npc_little_oozeAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if(pInstance && pInstance->GetBossState(DATA_ROTFACE) != IN_PROGRESS)
                {
                    me->DisappearAndDie();
                    return;
                }
...
class npc_big_ooze : public CreatureScript
{
    public:
        ...
        struct npc_big_oozeAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if(instance && instance->GetBossState(DATA_ROTFACE) != IN_PROGRESS)
                {
                    me->DisappearAndDie();
                    return;
                }
